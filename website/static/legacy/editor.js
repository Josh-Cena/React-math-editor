...
function renderinput() {
  const rawText = vm2.parseRawtext(input.value);
  MathJax.Hub.Queue(() => {
    const math = MathJax.Hub.getAllJax('output')[0];
    MathJax.Hub.Queue(['Text', math, rawText]);
  });
}
function showbox(num, show) {
  document.getElementById('Panel' + num).style.display = show
    ? 'block'
    : 'none';
}
function insertSymb(text) {
  if (input.selectionStart || input.selectionStart === '0') {
    ...
    const before = original.substring(0, startPos);
    const after = original.substring(endPos, original.length);
    const selected = original.substring(startPos, endPos);
    if (typeof text === 'object') {
      if (startPos !== endPos) {
        input.value = before + text.left + selected + text.right + after;
        input.selectionStart =
          startPos + text.left.length + selected.length + text.right.length;
      } else {
...
function showDialog(name, show) {
  ...
  document.getElementsByClassName('overlay')[0].style.display = show
    ? 'block'
    : 'none';
...
const vm = new Vue({
  ...
  data: {
    ...
    structures: [
      ...
      [
        ...
        {
          disp: '\\overrightarrow{xyz}',
          left: '\\overrightarrow{',
          right: '}',
        },
        {
          disp: '\\overleftrightarrow{xyz}',
          left: '\\overleftrightarrow{',
          right: '}',
        },
    ...
    arrows: [
      ...
      [
        ...
        '\\Updownarrow ',
        {
          disp: '\\stackrel{\\triangle}{\\longrightarrow}',
          left: '\\stackrel{',
    ...
    styles: [
      [
        {
          disp: '{\\displaystyle \\sum_a^b}',
          left: '{\\displaystyle ',
          right: '}',
        },
        { disp: '{\\textstyle \\sum_a^b}', left: '{\\textstyle ', right: '}' },
        {
          disp: '{\\scriptstyle \\sum_a^b}',
          left: '{\\scriptstyle ',
          right: '}',
        },
        {
          disp: '{\\scriptscriptstyle \\sum_a^b}',
          left: '{\\scriptscriptstyle ',
          right: '}',
        },
      ],
  ...
  methods: {
    add(charset, row, col) {
      ...
      switch (charset) {
        case 1:
          insertSymb(this.symbols[row][col]);
          break;
        case 2:
          insertSymb(this.operators[row][col]);
          break;
        case 3:
          insertSymb(this.structures[row][col]);
          break;
        case 4:
          insertSymb(this.arrows[row][col]);
          break;
        case 5:
          insertSymb(this.fonts[row][col]);
          break;
        case 6:
          insertSymb(this.styles[row][col]);
          break;
      }
    },
  },
...
const vm2 = new Vue({
    ...
    formulas: [
      {
        ...
        formula:
          '\\oint_C\\kappa_g\\,\\mathrm{d}s+\\iint_DK\\,\\mathrm{d}\\sigma=2\\pi-\\sum_{i=1}^n\\alpha_i',
      },
      {
        name: 'Fourier integral',
        formula:
          '\\lim_{N\\to+\\infty}\\frac1{2\\pi}\\int_{-N}^{N}\\hat{f}(\\lambda)\\,\\mathrm{e}^{\\mathrm{i}\\lambda x}\\,\\mathrm{d}\\lambda=f(x)',
      },
      {
  ...
  methods: {
    ...
    removeformula(index) {
      if (
        ...
        setTimeout(function () {
          const math = MathJax.Hub.getAllJax('history');
          for (let i = index; i < math.length; i++) {
    ...
    hide() {
      if (this.seen) foldHistory();
      else expandHistory();
      this.seen = !this.seen;
    },
    proceedSaving(confirm) {
      showDialog('Nameinput', false);
      const fname = document.getElementById('formulaname').value;
    ...
    proceedAddingSC(confirm) {
      ...
      if (confirm) {
        if (shortBox.value === '' || cutBox.value === '' || paramBox === '')
          ...
          let display = '\\' + shortBox.value;
          for (let i = 1; i <= paramBox.value; i++) display += '{#' + i + '}';
          this.shortcuts.push({
            short: '\\' + shortBox.value,
            cut: cutBox.value,
            parameter: paramBox.value,
            disp: display,
          });
        }
      }
  ...
  created() {
    let history = localStorage.getItem('webmathdata');
    if (history !== null) this.formulas = JSON.parse(history);
    history = localStorage.getItem('webmathdata2');
    if (history !== null) this.shortcuts = JSON.parse(history);
  },
  watch: {
    formulas(newVal) {
    ...
    fontsize(newVal) {
      if (newVal >= 10) {
        document
          ...
          .getElementsByTagName('div')[0].style.cssText =
          'display: table-cell;vertical-align: middle;font-size:' +
          newVal +
          'px !important';
        renderinput();
      }
...
window.onresize = function () {
  if (window.innerWidth <= 800) foldHistory();
  else expandHistory();
  vm2.seen = !vm2.seen;
};
